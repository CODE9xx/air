"""
KommoConnector — placeholder для V2.

Kommo — форк amoCRM с совместимым API v4. В V2 наследуем от `AmoCrmConnector`
и меняем только OAuth-домены. Сейчас — все методы raise NotImplementedError.
"""
...
_V2_NOT_IMPLEMENTED_MSG = (
    "KommoConnector.{method}: Kommo-интеграция планируется в V2. "
    "См. docs/product/ROADMAP.md."
)


class KommoConnector(CRMConnector):
    """Заглушка до V2. В MVP — недостижима через factory (только mock)."""

    provider: Provider = Provider.KOMMO
