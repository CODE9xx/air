...
=============================================================================
MIRROR of ``apps/api/app/db/url_translate.py`` — MUST stay in sync.
-----------------------------------------------------------------------------
Worker Docker image (``infra/docker/worker.Dockerfile``) не копирует
``apps/api`` в build-context, поэтому прямой импорт из ``app.db.url_translate``
невозможен без расширения build-scope. До тех пор, пока worker живёт в
отдельном Python-package, этот файл обязан побайтово (за исключением
этой шапки) повторять api-сторону. При изменении одного — менять второй
в том же коммите.
=============================================================================
...
* ``apps/worker/worker/lib/db.py``              — sync-engine в worker'е
  (DDL, Alembic-вызовы из RQ-job'ов).
